...
namespace Domain.appFacture
{
    public partial class OTOPVENDEUR
    {
        ...
        public OTOPVENDEUR(int id, string value)
        {
            this.IDVENDEUR = id;
            this.LIBELLESITE = value;
        }

        public List<OTOPVENDEUR> getAll()
        {
        ...
        public List<OTOPVENDEUR> getRestricted(List<long> ids)
        {
            using (EramEntities DB = new EramEntities())
            {
                var result = from c in DB.OTOPVENDEUR
                             where ids.Contains(c.IDVENDEUR)
                             select c;
                return result.Include("FACTENSEIGNE").ToList();
            }
        }
        //public List<OTOPVENDEUR> getAll(long IdUtilisateur)
        //{
        //    using (EramEntities DB = new EramEntities())
        //    {
        //        IQueryable<OTOPVENDEUR> result = from c in DB.OTOPVENDEUR
        ...
        //                result = from c in result where c.LIBELLESITE.ToLower().Contains(search) || c.FACTENSEIGNE.LIBELLEENSEIGNE.ToLower().Contains(search) || c.CODEAGRESSO.ToLower().Contains(search) || c.IDVENDEUR == searchid || c.CODEGOLD == searchid select c;
        //            else
        ...
        //            //    break;
        //            case 0:
        //                result = result.OrderByDescending(c => c.IDVENDEUR);
        //                break;
        //        }
        ...
        public OTOPVENDEUR getOne(int id)
        {
            using (EramEntities DB = new EramEntities())
            {
                var result = from c in DB.OTOPVENDEUR
                             where c.IDVENDEUR == id
                             select c;
        ...
        public OTOPVENDEUR getOneSsEns(int id)
        {
            using (EramEntities DB = new EramEntities())
            {
                var result = from c in DB.OTOPVENDEUR
        ...
        public OTOPVENDEUR getFromCodeGold(int codeGold)
        {
            using (EramEntities DB = new EramEntities())
            {
                var result = from c in DB.OTOPVENDEUR
        ...
        public void update(int IDVENDEUR, string LIBELLESITE, int IDENSEIGNE, int CODEGOLD, string CODEAGRESSO, string ADRLINE1, string ADRLINE2, string ICE, int? IDUTILISATEUR = null)
        {
            if (IDVENDEUR != 0)
            {
                ...
                using (EramEntities DB = new EramEntities())
                {
                    OTOPVENDEUR Site = new OTOPVENDEUR();
                    Site = Site.getOneSsEns(IDVENDEUR);
            ...
            else
            {
                ...
                using (EramEntities DB = new EramEntities())
                {
                    ...
                    Site.ADRLINE2 = ADRLINE2;
                    //Site.ICE = ICE;
                    DB.OTOPVENDEUR.Add(Site);
                    DB.SaveChanges();
                }
            }
        }

        public void updateDerniereExec(int IDVENDEUR,DateTime Dernieredate)
        {
                using (EramEntities DB = new EramEntities())
                {
                    OTOPVENDEUR Site = new OTOPVENDEUR();
                    Site = Site.getOne(IDVENDEUR);
