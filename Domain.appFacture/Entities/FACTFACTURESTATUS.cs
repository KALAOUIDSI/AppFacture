...
namespace Domain.appFacture
{
    public partial class FACTFACTURESTATUS
    {
        ...
        public FACTFACTURESTATUS(int id, string value)
        {
            this.STATUS = (short)id;
            this.LIBELLE = value;
        }
        public List<FACTFACTURESTATUS> getAllStatus()
        {
            using (EramEntities DB = new EramEntities())
            {
                IQueryable<FACTFACTURESTATUS> result = from c in DB.FACTFACTURESTATUS
                                                       select c;
